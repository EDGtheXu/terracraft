...
public class eaterOfWorldRenderer extends GeoEntityRenderer<eaterOfWorld> {
    public eaterOfWorldRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new geoNormalModel<>("worm_eater_of_world"));
    }

    @Override
    public void preRender(PoseStack poseStack, eaterOfWorld animatable, BakedGeoModel model, @Nullable MultiBufferSource bufferSource, @Nullable VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int colour) {
        super.preRender(poseStack, animatable, model, bufferSource, buffer, isReRender, partialTick, packedLight, packedOverlay, colour);
        poseStack.scale(3.5f,3.5f,3.5f);
        poseStack.translate(0,0,0);

        poseStack.mulPose(Axis.YN.rotationDegrees(-90));
        System.out.println(animatable.yRotO-90);
        System.out.println(animatable.xRotO);
